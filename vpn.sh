...
# Buat config client Umobile
cat > /etc/openvpn/Umobile-${domain}.ovpn <<-END
############# Lukaku ##############
####### TELEGRAM : @coxoox #######
client
dev tun
proto tcp
remote xxxxxxxxx:1194www.mobilelegends.com.pay-dcb.u.com.my.wap.u.com.my/
...
sed -i $MYIP2 /etc/openvpn/Umobile-${domain}.ovpn;

# Buat config client Maxis A
cat > /etc/openvpn/Maxis-A-${domain}.ovpn <<-END
############# Lukaku ##############
####### TELEGRAM : @coxoox #######
client
dev tun
proto tcp
remote www.viu.com.${domain} 1194
resolv-retry infinite
route-method exe
nobind
persist-key
...
sed -i $MYIP2 /etc/openvpn/Maxis-A-${domain}.ovpn;

# Buat config client Maxis B
cat > /etc/openvpn/Maxis-B-${domain}.ovpn <<-END
############# Lukaku ##############
####### TELEGRAM : @coxoox #######
client
dev tun
proto tcp
remote stream-hk.viu.com.${domain} 1194
resolv-retry infinite
route-method exe
nobind
persist-key
...
sed -i $MYIP2 /etc/openvpn/Maxis-B-${domain}.ovpn;
